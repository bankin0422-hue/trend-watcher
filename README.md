...
CDPR関連（Cyberpunk 2077 / Orion / Witcher / エッジランナーズS2）の公式発表・リーク・コミュニティの盛り上がりを無人監視し、動画化すべき事象を検知から30分以内にDiscordへ通知する。
...
4. スコアで分類: **S級 → 即時通知（メンション付き可）** / A級 → 1時間毎ダイジェスト / B級 → 日次ダイジェスト
...
2. Discordサーバーで `#速報-s級` と `#ダイジェスト` チャンネルを作成し、それぞれWebhookを発行
3. リポジトリの Settings → Secrets and variables → Actions に登録:

   | Secret | 内容 | 必須 |
   |---|---|---|
   | `DISCORD_WEBHOOK_S` | #速報-s級 のWebhook URL | ✅ |
   | `DISCORD_WEBHOOK_DIGEST` | #ダイジェスト のWebhook URL（省略時はS級と同じ先） | - |
   | `DISCORD_MENTION` | S級通知のメンション（`@everyone` や `<@&ロールID>`） | - |
...
python -m src.main --dry-run   # Discordへ送信せず通知内容を標準出力に表示
```

実際にDiscordへ飛ばすテスト:

```powershell
$env:DISCORD_WEBHOOK_S = "https://discord.com/api/webhooks/..."
python -m src.main
```

## 受け入れテスト手順（要件 §10）

1. **15分以内に通知**: `keywords.yaml` のS級に入っている動作確認用キーワード `TW-TEST` を含むスレをテスト用subredditに投稿するか、辞書に現在ニュースに載っている語を一時追加 → 次のActions実行で通知が届くこと。確認後 `TW-TEST` は削除してよい
...
5. **月額0円**: GitHub Actions無料枠 + Discord Webhookのみ。AI APIの定常呼び出しなし（台本骨子は通知を受けた人間がClaude Proで生成）
